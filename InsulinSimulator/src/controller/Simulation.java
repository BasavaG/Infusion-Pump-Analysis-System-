...
import configuration.Battery1;
import configuration.DosageCalculation;
...
public class Simulation {
	...
	public DosageCalculation insulinGlucagonCalculation;
	public Battery1 batteryClass;
    ...
    void initialize() {		
        ...
        timeline.getKeyFrames().add(new KeyFrame(Duration.millis(300000), x -> changeBatteryIcon()));
        ...
        addMessage("Auto Mode turned on ", Color.GAINSBORO);
    }

	// The method to add messages to list item
	public static void addMessage(String message) {
		addMessage(message, Color.GAINSBORO);
	}
	...
	public void InitialiseBSLLineChart() {
		...
		yAxis.setLabel("Sugar Level in Blood");
		
		
		if(bloodSugarLevelChart != null) {
			addMessage("The Blood Sugar Level is Stable :  " +  StaticValues.CurrentBSL + "mg/dl", Color.DARKSEAGREEN);
		...
		if(checkForBsl) {	
			if(isAutoMode) {
				...
				if(StaticValues.TempBSL > 120 && StaticValues.CurrentBSL > 120) {
					...
					bsl = SugarCalculation.getInstance().bslOnGlucagonDosage(CheckForGlucagonLevel(true));
					StaticValues.CurrentBSL = bsl;
					
				}
				else if(StaticValues.CurrentBSL < 120 && autoInjectionStarted){
					bsl = SugarCalculation.getInstance().bslOnIdeal();
				}
				else {
					bsl = SugarCalculation.getInstance().bslAfterActivity(StaticValues.CarbohydrateIntake , 1 * increasingCarboCounter);
				}
				...
				if(inManualInject) {
					...
					if(StaticValues.CurrentBSL > 120) {
						bsl = SugarCalculation.getInstance().bslOnInsulinDosage(CheckForInsulinLevel(false));
					}
					else if(StaticValues.CurrentBSL < 70) {
						bsl = SugarCalculation.getInstance().bslOnGlucagonDosage(CheckForGlucagonLevel(false));
					}
	...
	private void showChangeinBSLMessages(double bsl) {
		...
		if((bsl >= StaticValues.MinimumBloodSugarLevel && bsl <= StaticValues.MaximumBloodSugarLevel) && showStableBslMsg) {
			addMessage("Blood Sugar Level is back to stable!", Color.DARKSEAGREEN);
	...
	private double CheckForInsulinLevel(boolean isAuto) {
		double insulin = 0;
		if(isAuto) {
			insulin = DosageCalculation.getInsulinDosageValue(StaticValues.CurrentBSL);
		}
		else {
		...
		if(insulin > 0) {
			...
			double bslAfterInsulin = SugarCalculation.getInstance().bslOnInsulinDosage(insulin);
	...
	private double CheckForGlucagonLevel(boolean isAuto) {
		...
		if(isAuto) {
			glucagon =	DosageCalculation.getGlucagonDosageValue(StaticValues.CurrentBSL);
		}
		else {
		...
		if(glucagon > 0) {
			...
			StaticValues.CurrentBSL = SugarCalculation.getInstance().bslOnGlucagonDosage(glucagon);
			...
			double bslAfterGlucagon = SugarCalculation.getInstance().bslOnGlucagonDosage(glucagon);
	...
	public void autoModeClick(ActionEvent event) throws IOException {	
		...
		Simulation.autoInjectionStarted = Simulation.isAutoMode ? true: false;
		btnAuto.setStyle(isAutoMode ? "-fx-background-color:  #90EE90" : "-fx-background-color: black;");
	}
	...
	public void insulinRefillClick(MouseEvent event) {
		...
		addMessage("Insulin Bank is Refilled! ", Color.DARKSEAGREEN);
	...
	public void glucagonRefillClick(MouseEvent event) {
		...
		addMessage("Glucagon Bank is Refilled! ", Color.DARKSEAGREEN);
	}
	...
	public void batteryChargeClick(MouseEvent event) {
		...
		addMessage("The Battery is fully charged", Color.DARKSEAGREEN);
	}
	
	// the Method closes the application
	public void closeApplication(MouseEvent event) {
		if(buildAlertMessage(AlertType.CONFIRMATION, "Confirmation Dialog", "System Shutdown", "The system will shutdown and Emergency services will not be available. Do you want to continue?")) {
			System.exit(0);
		}
	}
	...
	public void btnGlucagonInj(ActionEvent event) {
		...
		double glucagon = Double.parseDouble(glucagonInjected);
		if(glucagon > 2) {
			Alert alert = new Alert(AlertType.WARNING);
			alert.setTitle("Alert Dialog");
	        alert.setHeaderText("Glucagon");
